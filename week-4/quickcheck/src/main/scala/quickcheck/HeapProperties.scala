...
trait HeapProperties(val heapInterface: HeapInterface):
  ...
  val deleteAllProducesSortedList: (String, Prop) =
    ...
    def check(heap: List[Node]): Boolean =
      ...
      if isEmpty(heap) || isEmpty(deleteMin(heap)) then
        ...
        val checked: Boolean = x2 >= x1 && check(heap2)
        checked
  ...
  val meldingSmallHeaps: (String, Prop) =
    ...
    forAll { (x: Int, y: Int) =>
      ...
      val meldedHeap: List[Node] = meld(insert(x, insert(x, empty)), insert(y, insert(y, empty)))
  ...
  val meldingHeaps: (String, Prop) =
    def check(m: List[Node], h1: List[Node], h2: List[Node]): Boolean =
      // 1. the melded heap is empty
      if m.isEmpty then
        h1.isEmpty && h2.isEmpty // just 1 line, very short answer
      else 
        val minMeld = findMin(m)
        //  2. the minimum of the melded heap is the minimum of the first source heap
        if !h1.isEmpty && minMeld == findMin(h1) then
          // (then, check that after...) just 1 line, single expression. Use recursion!
          check(deleteMin(m), deleteMin(h1), h2)
        else
          if !h2.isEmpty && minMeld == findMin(h2) then  
            //  3. the minimum of the melded heap is the minimum of the second source heap
            // (almost the same as 2, should be just 2 lines, copy/paste/edit)
            check(deleteMin(m), h1, deleteMin(h2))
          else 
            //  4. all the other cases (which should not happen)
            false
