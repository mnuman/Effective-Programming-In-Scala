...
case class WikiResult[A](value: Future[Either[Seq[WikiError], A]]):
  ...
  def map[B](f: A => B)(using ExecutionContext): WikiResult[B] =
    ...
    WikiResult(
      value.map(v =>
        v match
          case Left(anyValue) => Left(anyValue)
          case Right(aValue)  => Right(f(aValue))
      )
    )
...
object WikiResult:
  ...
  def traverse[A, B](as: Seq[A])(f: A => WikiResult[B])(using
      ...
      as.map(f)
      .foldLeft(
        WikiResult.successful[Seq[B]](Seq.empty[B])
        )
          (
            (aggregatedResults, curr) => { 
              aggregatedResults.zip(curr.map(Seq(_))).map(e => e._1 ++ e._2)
            }
          )
end WikiResult
